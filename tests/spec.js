...
var spec_data = fs_1.readFileSync(path_1.join(__dirname, 'spec.yaml'), { encoding: 'utf8' });
    ...
    pointers.forEach(pointer => {
        describe(`pointer "${pointer.path}"`, () => {
            it('should evaluate to expected output', () => {
                var actual = pointer_1.Pointer.fromJSON(pointer.path).evaluate(obj).value;
...
describe('Specification patches:', () => {
    ...
    spec_patch_results.forEach(spec_patch_result => {
        describe(spec_patch_result.name, () => {
            ...
            var results = index_1.applyPatch(actual, spec_patch_result.patch);
...
describe('Specification diffs:', () => {
    spec_patch_results.filter(item => item.diffable).forEach(spec_patch_result => {
        ...
        describe(spec_patch_result.name, () => {
            ...
            var actual = index_1.createPatch(spec_patch_result.input, spec_patch_result.output);
